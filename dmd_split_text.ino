...
int i;            
char greeting[] = "www.collaborizm.com";
/*--------------------------------------------------------------------------------------
...
void loop(void)
{
 ...
 //prepares the display to print our message
   ...
   //displays the static heading text
   for (byte x=0;x<DISPLAYS_ACROSS;x++) {
     for (byte y=0;y<DISPLAYS_DOWN;y++) {
   dmd.drawString(  8+(32*x),  0, "C", 3, GRAPHICS_NORMAL ); 
     }
   }
   
   //displays the scrolling message
   for(int i=32;i>=-int(sizeof(message));i--){
    for (byte x=0;x<DISPLAYS_ACROSS;x++) {
     for (byte y=0;y<DISPLAYS_DOWN;y++) {
       dmd.drawString(  i+(32*x),  8, message, sizeof(message), GRAPHICS_NORMAL );
     }
    }
    delay(50); //decides the scrolling speed
   }
  dmd.clearScreen(true);
}
